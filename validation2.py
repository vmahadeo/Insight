# The purpose of this file is to validate the model.  We are comparing article to transcript cosine
# similarity distributions to title to title distributions.
...
# New tokenizer that includes stemming
def tokenize(text):
	tokens = word_tokenize(text)
	stems = stem_tokens(tokens, stemmer)
	return stems

# Build corpus and create TFIDF vectors.
def createVectors( docs ):
	all_docs = []
	all_titles = []
	for d in docs:
	...
	return tfidf_docs, tfs_docs

if __name__ == '__main__':
	docs = initMongo()
	tfidf_docs, tfs_docs = createVectors(docs)

	# Collect articles and compare cosine similarities.
	NUM_ARTICLES = 1000
